...
<div class="bg-light p-6 rounded-2xl shadow-normalBox mt-8">
    <div class="flex flex-col items-center  ">  <!-- Centrato orizzontalmente, ma verso l'alto -->
        ...
        <form action="/ordinante/invia-richiesta" method="POST" class="max-w-xl w-full" id="formRichiesta">
            ...
            <ul class="space-y-4">
                <#list caratteristicheList as caratteristica>
                    <li class="flex flex-row items-center justify-between gap-4 bg-white shadow rounded-lg p-4 rounded-2xl bg-light ">
                        <h2 class="text-xl font-semibold text-bluScuro">${caratteristica.nome!}</h2>
                        ...
                        <span class="text-gray-600 text-bluScuro">${caratteristica.getMisura()! "N/A"}</span>

                        <!-- Checkbox per disabilitare l'input -->
                        <label class="flex items-center gap-2 text-bluScuro">
                ...
                <li class="flex flex-row items-center justify-between gap-4 bg-white text-bluScuro shadow rounded-lg p-4 rounded-2xl bg-light ">
                    <h2 class="text-xl font-semibold ">Note</h2>
                    <input type="text" name="note" class="  rounded-md px-4 py-2 flex-1 border border-gray-300"
                           placeholder="Inserisci nota (OPZIONALE)">
                </li>

            </ul>
            <!-- Messaggio di errore nascosto -->
            <div id="errorMessage" class="text-red-500 text-center mt-4 hidden">Per favore, compila tutte le
                caratteristiche o seleziona "Indifferente".
            </div>
            <div class="flex flex-row justify-end">
                <button type="submit"
                        class="mt-6 bg-verde hover:bg-green-700 shadow-buttonBox hover:shadow-buttonBoxHover text-white font-semibold py-3 px-6 rounded-full ">
                    Invia Richiesta
                </button>
            </div>
        </form>
